...
- **播放控制需要 `user-modify-playback-state` scope,而改 scope 會讓已存的 `refresh_token` 失效。**
  ...
  **動 `SCOPE` 常數時這段守門不能省。**
  ...
  - 403 = 免費帳號或 scope 不對 → 整排控制鈕收起來並提示一次,不要每按一次吐一次。
  - **404 不等於「Spotify 沒開」,先自己救一次再說**:暫停一陣子後裝置只是從活躍名單掉出去
    (`/me/player` 回 204、控制端點回 404),app 其實還活著 —— iOS 的控制中心那時仍留著播放狀態,
    `/me/player/devices` 也照樣列得到它,轉移過去就會開始播。所以 `control()` 收到 404 會先跑一次
    `transferToDevice()`,失敗才顯示救援鈕;只吐錯誤叫使用者去按救援鈕等於同一件事要按兩下。
