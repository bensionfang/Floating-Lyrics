...
// 這支刻意**不加 defer** —— 它要在原本內嵌的那個位置同步執行,順序與時機才跟改版前一致。
        ...
        const tourSteps = [
            ...
            { page: '/',       el: '#menu-dots-btn', title: '設定選單',   text: '常用的開關直接列在上面：日文假名、片假名標平假名、顯示中文翻譯、文字大小、歌詞對齊、優先搜尋來源。下半部收成幾個小節——靈動島、音訊來源、AI 讀音校正、自訂快捷鍵、聆聽紀錄與資料，點一下往旁邊展開。讀不了片假名的話打開「片假名標平假名」，サヨナラ 上方會多標一行 さよなら；打開「顯示中文翻譯」則會在每句日文歌詞下面多一行中文，兩者網頁與靈動島同時生效。' },
            ...
            { page: '/',       el: '.player-right',  title: '工具列',     text: '由左至右：備選歌詞、段落循環、編輯假名、AI 讀音校正、重新載入歌詞、桌面靈動島、放大模式。每顆都可以在設定選單的「自訂快捷鍵」裡改快捷鍵,也能按眼睛圖示把不需要的工具從這排隱藏。接下來由左到右細講。' },
            ...
            { page: '/',       el: '#desktop-toggle-btn', title: '桌面靈動島', text: '開啟一個永遠置頂的懸浮歌詞條，切到別的視窗也看得到歌詞，適合一邊工作一邊聽歌。再按一次關閉，它也會跟著 Kanaric 一起結束。島可以直接拖著走，拖到螢幕最上緣會自動吸附；在島上單擊一下可以在「貼齊頂端」與「浮在下面一點」之間切換。歌詞行數、第二行要顯示下一句還是本句翻譯、透明度、鎖定位置、重置位置、開機自動顯示都在設定選單的「靈動島」小節。' },
        ...
        function renderTourStep() {
            ...
            // 而魔杖沒設 API key 時是 SSR 就整顆不長 (連元素都沒有)。兩種都退回「無目標」那條:
            // 卡片置中、不畫框,並在說明下面附一行原因。步驟不跳過 —— 使用者仍該知道有這個功能。
            ...
            if (targetMissing) {
                stepText += '<div class="tour-note">這個東西目前不在畫面上。工具列的按鈕可以在設定選單的'
                    + '「自訂快捷鍵」用眼睛圖示收起來,點回來就會出現;「AI 讀音校正」的魔杖則要先在'
                    + '「AI 讀音校正」小節填好 API Key 再重新載入頁面。</div>';
            }
            po.innerHTML = `
